...
class GraphDataset:
    def __init__(self, graph_save_path, data_id, split_id):
    ...
    def load(self):
        if self._data_id in [1, 2]:
            [rmol_dict, pmol_dict, reaction_dict] = np.load(
                "%s/dataset_%d_%d.npz"
                % (self.graph_save_path, self._data_id, self._split_id),
                allow_pickle=True,
            )["data"]
        elif self._data_id == 3:
            [rmol_dict, pmol_dict, reaction_dict] = np.load(
                "%s/test_%d.npz" % (self.graph_save_path, self._split_id),
                allow_pickle=True,
            )["data"]
